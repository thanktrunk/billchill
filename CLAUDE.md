...
Before writing any code, review and follow the coding conventions and styling rules defined in this guide:

- **Coding conventions** — server actions, auth, group access, balances, locale, DB amounts, i18n (see below).
- **Code quality rules** — reuse utilities (`currencySymbol`, `cn()`, `bc-ui` primitives) before writing new ones; fetch data server-side; derive values instead of duplicating state; pass typed args to server actions; pre-index data used in render loops.
- **Styling rules** — Tailwind classes first; no inline `style={{}}` for static values; use CSS variable shorthand (`bg-(--bc-ink)`); prefer scale tokens over arbitrary values; use `cn()` for conditional classes; active/hover via pseudo-classes only.

...
function GroupsHome({ state, theme, accent, T, lang, onOpenGroup, onNewGroup }) {
  ...
  const rows = groups.map((g) => {
    ...
    const last = expenses.length
      ? expenses.reduce((a, b) => (a.created_at > b.created_at ? a : b))
      : null;
    return { g, myBal, members, lastTime: last?.created_at, expCount: expenses.length };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <BCTopBar theme={theme}
        left={<div style={{
          fontFamily: "'Newsreader', serif", fontSize: 28, color: theme.ink,
          letterSpacing: '-0.015em', paddingLeft: 6, whiteSpace: 'nowrap',
        }}>{T('brand')}</div>}
      ...
      <div style={{ padding: '8px 16px 0' }}>
        <div style={{
          background: theme.ink, color: theme.bg, borderRadius: 28,
          padding: '20px 22px', position: 'relative', overflow: 'hidden',
        }}>
          <div style={{
            fontFamily: 'Be Vietnam Pro', fontSize: 11, color: 'rgba(245,241,234,0.55)',
            textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: 6,
          }}>{T('home.your_balance')}</div>

          <div>
            <div style={{
              fontFamily: "'Newsreader', serif", fontSize: 60, lineHeight: 0.95,
          ...
          <div style={{ display: 'flex', gap: 14, marginTop: 18 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 11, opacity: 0.5, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{T('home.owed_to_you')}</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 16, fontWeight: 500, marginTop: 4, color: '#9CC8A8', fontVariantNumeric: 'tabular-nums' }}>
                {heroCurrency}{totalOwed.toFixed(2)}
              </div>
            </div>
            <div style={{ width: 1, background: 'rgba(245,241,234,0.16)' }} />
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 11, opacity: 0.5, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{T('home.you_owe')}</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 16, fontWeight: 500, marginTop: 4, color: '#F2A788', fontVariantNumeric: 'tabular-nums' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '22px 22px 10px' }}>
        <SectionLabel theme={theme}>{T('home.groups_section')}</SectionLabel>
        <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 12, color: theme.muted }}>{T('home.active', [rows.length])}</div>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px 140px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {rows.map((r) => <GroupRow key={r.g.id} row={r} theme={theme} T={T} lang={lang}
          state={state} onClick={() => onOpenGroup(r.g.id)} />)}
        {rows.length === 0 && (
          <div style={{ padding: '40px 20px', textAlign: 'center', color: theme.muted, fontFamily: 'Be Vietnam Pro' }}>
            {T('home.empty')}
          </div>
        )}
      </div>
    ...
    <Card theme={theme} onClick={onClick} padded={false} style={{ padding: '14px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 16, color: theme.ink, letterSpacing: '-0.01em',
            whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
          }}>{g.name}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
            <AvatarStack members={members} size={20} max={4} theme={theme} />
            {lastTime && (
              <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 12, color: theme.muted }}>· {bcRelativeTimeL(lastTime, lang)}</div>
            )}
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          {settled ? (
            <div style={{
              fontFamily: 'Be Vietnam Pro', fontSize: 12, color: theme.muted, letterSpacing: '0.06em', textTransform: 'uppercase',
            }}>{T('home.settled')}</div>
          ) : (
            <React.Fragment>
              <div style={{
                fontFamily: 'Be Vietnam Pro', fontSize: 10, color: theme.muted, letterSpacing: '0.08em',
                textTransform: 'uppercase', whiteSpace: 'nowrap',
              }}>{isOwed ? T('home.youre_owed') : T('home.you_owe_short')}</div>
              <div style={{
                fontFamily: "'Newsreader', serif", fontSize: 26, lineHeight: 1,
                color: isOwed ? theme.pos : theme.neg,
                fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.01em', marginTop: 2,
              }}>{sym}{Math.abs(myBal).toFixed(2)}</div>
            </React.Fragment>
          )}
        </div>
      </div>
...
function GroupGlyph({ name, size = 44 }) {
  ...
  return (
    <div style={{
      ...
      fontFamily: "'Newsreader', serif", fontSize: size * 0.55, letterSpacing: '-0.02em',
      flexShrink: 0,
    }}>{ch}</div>
  );
}
...
function GroupDetail({ state, theme, accent, T, lang, groupId, onBack, onAddExpense, onSettleUp }) {
  ...
  const expenses = bcGroupExpenses(state, groupId).slice().sort((a, b) => b.created_at.localeCompare(a.created_at));
  const settlements = bcGroupSettlements(state, groupId).slice().sort((a, b) => b.settled_at.localeCompare(a.settled_at));
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '6px 16px 0' }}>
        <Card theme={theme} padded={false} style={{
          padding: '16px 18px', background: theme.ink, color: theme.bg, border: 'none',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div>
              <div style={{
                fontFamily: 'Be Vietnam Pro', fontSize: 11, opacity: 0.55, textTransform: 'uppercase', letterSpacing: '0.14em',
              }}>{myBal > 0.005 ? T('group.youre_owed') : myBal < -0.005 ? T('group.you_owe') : T('group.all_settled')}</div>
              <div style={{ marginTop: 6 }}>
                {Math.abs(myBal) < 0.005 ? (
                  <div style={{ fontFamily: "'Newsreader', serif", fontSize: 44, lineHeight: 0.95, letterSpacing: '-0.02em' }}>
                    {sym}0.00
                  </div>
                ) : (
                  <Money value={Math.abs(myBal)} currency={sym} size={44}
                    color={myBal > 0 ? '#9CC8A8' : '#F2A788'} family="serif" animate={false} />
                )}
              </div>
            </div>
            <button onClick={onSettleUp} className="bc-tap" style={{
              background: 'rgba(245,241,234,0.12)', color: theme.bg, border: 'none',
              padding: '12px 18px', borderRadius: 999, cursor: 'pointer',
              fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 13, letterSpacing: '-0.005em',
      ...
      <div style={{ padding: '14px 0 8px' }}>
        <BCTabs theme={theme} active={tab} onChange={setTab}
          tabs={[
            { k: 'expenses', label: T('group.tab_expenses', [expenses.length]) },
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 120px' }}>
        {tab === 'expenses' && (
          <ExpensesList expenses={expenses} settlements={settlements} state={state} theme={theme}
            currencySymbol={sym} myMid={myMid} T={T} lang={lang} />
        )}
        {tab === 'balances' && (
          <BalancesView members={members} balances={balances} simplified={simplified}
            state={state} theme={theme} currencySymbol={sym} myMid={myMid}
            T={T} onSettleUp={onSettleUp} />
        )}
      </div>
      ...
      {groups.map((grp) => (
        <div key={grp.day}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {grp.items.map((it) => it.kind === 'expense'
              ? <ExpenseRow key={it.e.id} expense={it.e} state={state} theme={theme} currencySymbol={currencySymbol} myMid={myMid} T={T} />
              : <SettlementRow key={it.s.id} settlement={it.s} state={state} theme={theme} currencySymbol={currencySymbol} T={T} lang={lang} />
            )}
          </div>
        </div>
      ))}
  ...
  return (
    <Card theme={theme} padded={false} style={{ padding: '12px 14px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 14.5, color: theme.ink, letterSpacing: '-0.005em',
            whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
          }}>{expense.description}</div>
          <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 12, color: theme.muted, marginTop: 2 }}>
            {T(sharesKey, [payerName, splits.length])}
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{
            fontFamily: "'Newsreader', serif", fontSize: 22, lineHeight: 1, color: theme.ink,
            fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.01em',
          }}>{currencySymbol}{expense.amount.toFixed(2)}</div>
          {(lent > 0.005 || owe > 0.005) && (
            <div style={{
              fontFamily: "'JetBrains Mono', monospace", fontSize: 11, marginTop: 3,
              color: iPaid ? theme.pos : theme.neg, letterSpacing: '0.02em',
            }}>
    ...
    <Card theme={theme} padded={false} style={{ padding: '12px 14px', background: theme.chip, border: 'none' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{
          ...
          color: theme.ink, border: `1px dashed ${theme.hair}`,
        }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 14.5, color: theme.ink, letterSpacing: '-0.005em',
          }}>{T('group.paid_to', [fromName, toName])}</div>
          <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 12, color: theme.muted, marginTop: 2 }}>
        ...
        <div style={{
          fontFamily: "'Newsreader', serif", fontSize: 22, lineHeight: 1, color: theme.ink,
          fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.01em',
        }}>{currencySymbol}{settlement.amount.toFixed(2)}</div>
      </div>
    </Card>
  );
}
...
function BalancesView({ members, balances, simplified, state, theme, currencySymbol, myMid, T, onSettleUp }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div>
        <div style={{ padding: '0 4px 8px' }}><SectionLabel theme={theme}>{T('group.member_balances')}</SectionLabel></div>
          ...
          {members.map((m, i) => {
            ...
            return (
              <div key={m.id} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{
                    fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 14.5, color: theme.ink,
                  }}>{name}{isMe && <span style={{
                    fontSize: 10, marginLeft: 8, padding: '2px 7px', borderRadius: 999,
                    background: theme.chip, color: theme.muted, letterSpacing: '0.08em',
                  }}>{T('group.you_label')}</span>}</div>
                  <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 11, color: theme.muted, marginTop: 2, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
      ...
      {simplified.length > 0 && (
        <div>
          <div style={{ padding: '0 4px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 11, color: theme.muted, letterSpacing: '0.04em' }}>
            ...
            {simplified.map((s, i) => {
              ...
              return (
                <Card key={i} theme={theme} padded={false} style={{
                  padding: '12px 14px', background: involvesMe ? theme.surface : theme.chip,
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0, paddingLeft: 6 }}>
                      <div style={{
                        fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 14, color: theme.ink, letterSpacing: '-0.005em',
                      }}>{T('group.pays', [fromName, toName])}</div>
                    </div>
                    <div style={{
                      fontFamily: "'Newsreader', serif", fontSize: 22, lineHeight: 1, color: theme.ink,
                      fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.01em',
                    }}>{currencySymbol}{s.amount.toFixed(2)}</div>
                  </div>
                </Card>
              );
            })}
          ...
          <div style={{ marginTop: 12 }}>
            <BCButton variant="quiet" theme={theme} full onClick={onSettleUp}
...
function AddExpense({ state, theme, accent, T, lang, groupId, draft, setDraft, onBack, onSave }) {
  ...
  const onKey = (k) => {
    setDraft((d) => {
      ...
      else if (k === '.') { if (!s.includes('.') && s.length > 0) s += '.'; }
      else {
  ...
  if (step === 'amount') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <BCTopBar theme={theme} title={T('add.title')} subtitle={g.name}
        ...
        <div style={{ padding: '8px 22px 0' }}>
          <input
            ...
            style={{
              ...
              fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 18, color: theme.ink, letterSpacing: '-0.01em', padding: '6px 0',
            }} />
          <div style={{ height: 1, background: theme.softhair, marginTop: 2 }} />
        </div>

        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '20px 24px' }}>
        ...
        <div style={{ padding: '4px 18px 18px' }}>
          <BCButton
            variant="accent" full theme={theme} accent={accent}
            disabled={!(amount > 0)}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 16px 8px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{
          ...
          gap: 12, padding: '4px 4px',
        }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{
              fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 16, color: theme.ink, letterSpacing: '-0.005em',
              whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
            }}>{draft.description || T('add.untitled')}</div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: theme.muted, marginTop: 2, letterSpacing: '0.04em' }}>
              {bcShortDateL(draft.date || '2026-05-22', lang)}
            </div>
          </div>
          <div style={{
            fontFamily: "'Newsreader', serif", fontSize: 36, lineHeight: 1, color: theme.ink,
            fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.015em',
          }}>{sym}{amount.toFixed(2)}</div>
        </div>

        <div>
          <div style={{ padding: '0 4px 8px' }}><SectionLabel theme={theme}>{T('add.paid_by')}</SectionLabel></div>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '0 4px' }}>
            {members.map((m) => {
              const sel = m.id === draft.paid_by;
              const name = bcMemberName(state, m, T);
              return (
                <button key={m.id} onClick={() => setDraft((d) => ({ ...d, paid_by: m.id }))}
                  className="bc-tap" style={{
        ...
        <div>
          <div style={{ padding: '0 4px 8px' }}><SectionLabel theme={theme}>{T('add.category')}</SectionLabel></div>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '0 4px 4px' }}>
            {Object.entries(BC_CATEGORIES).map(([k, c]) => {
              ...
              return (
                <button key={k} onClick={() => setDraft((d) => ({ ...d, category: k }))}
                  className="bc-tap" style={{
                  flexShrink: 0, border: 'none', cursor: 'pointer',
                  ...
                  <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 11, color: sel ? theme.ink : theme.muted, whiteSpace: 'nowrap' }}>{T('cat.' + k)}</div>
                </button>
              );
            })}
          </div>
        ...
        <div>
          <div style={{ padding: '0 4px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: theme.muted, letterSpacing: '0.04em' }}>
          ...
          <Card theme={theme} padded={false}>
            {members.map((m, i) => {
              ...
              const name = bcMemberName(state, m, T);
              return (
                <div key={m.id} onClick={() => toggleSplitMember(m.id)} className="bc-tap" style={{
                  display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px',
                  borderTop: i === 0 ? 'none' : `1px solid ${theme.softhair}`, cursor: 'pointer',
                }}>
                  <Avatar name={name} seed={m.id} size={32} />
                  <div style={{ flex: 1, minWidth: 0, fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 14.5, color: theme.ink }}>
                    {name}
                  </div>
                  <div style={{
                    fontFamily: "'JetBrains Mono', monospace", fontSize: 12,
                    color: has ? theme.ink : theme.muted, fontVariantNumeric: 'tabular-nums',
                  }}>{has ? `${sym}${perPerson.toFixed(2)}` : '—'}</div>
                  <div style={{
                    width: 24, height: 24, borderRadius: 6,
                    background: has ? accent : 'transparent', border: has ? 'none' : `1.6px solid ${theme.hair}`,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <div style={{ padding: '4px 16px 16px' }}>
        <BCButton variant="accent" full theme={theme} accent={accent}
...
function SettleUp({ state, theme, accent, T, groupId, onBack, onSave }) {
  ...
  if (done) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', alignItems: 'center', justifyContent: 'center', padding: '0 24px' }}>
        <div style={{
          width: 80, height: 80, borderRadius: 999, background: theme.pos, color: '#fff',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        <div style={{
          marginTop: 28, fontFamily: "'Newsreader', serif", fontSize: 40, color: theme.ink,
          letterSpacing: '-0.02em', textAlign: 'center', lineHeight: 1.05,
        }}>{T('settle.done_title')}</div>
        <div style={{ marginTop: 10, fontFamily: 'Be Vietnam Pro', fontSize: 15, color: theme.muted, textAlign: 'center' }}>
          {T('settle.done_subtitle')}
        </div>
        <div style={{ marginTop: 40, width: '100%' }}>
          <BCButton variant="primary" full theme={theme} onClick={onBack}>{T('settle.back_to_group')}</BCButton>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <BCTopBar theme={theme} title={T('settle.title')} subtitle={g.name}
        left={<IconBtn name="close" theme={theme} onClick={onBack} />} right={null} />

      <div style={{ padding: '12px 16px 0' }}>
        <Card theme={theme} padded={false} style={{ padding: '14px 16px' }}>
          <SettleRow label={T('settle.from')} memberId={fromMid} onChange={setFromMid} members={members} state={state} theme={theme} T={T} />
          <div style={{ display: 'flex', justifyContent: 'center', padding: '2px 0' }}>
            <button onClick={() => { setFromMid(toMid); setToMid(fromMid); }} className="bc-tap" style={{
              border: 'none', background: theme.chip, color: theme.ink, cursor: 'pointer',
      ...
      <div style={{ padding: '4px 16px 16px' }}>
        <BCButton variant="accent" full theme={theme} accent={accent}
          ...
          onClick={() => { onSave({ from: fromMid, to: toMid, amount }); setDone(true); }}
          icon={<BCIcon name="check" size={18} color="#fff" stroke={2.2} />}>
          {T('settle.record')}
        </BCButton>
      </div>
...
function SettleRow({ label, memberId, onChange, members, state, theme, T }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0' }}>
      <div style={{ width: 36, fontFamily: 'Be Vietnam Pro', fontSize: 11, color: theme.muted, textTransform: 'uppercase', letterSpacing: '0.12em' }}>{label}</div>
      <div style={{ flex: 1, display: 'flex', gap: 8, overflowX: 'auto' }}>
        {members.map((mm) => {
          const sel = mm.id === memberId;
          const name = bcMemberName(state, mm, T);
          return (
            <button key={mm.id} onClick={() => onChange(mm.id)} className="bc-tap" style={{
              flexShrink: 0, border: 'none', cursor: 'pointer',
              background: sel ? theme.ink : theme.chip,
              color: sel ? theme.bg : theme.ink,
...
function ActivityScreen({ state, theme, accent, T, lang, onMarkRead, onOpenGroup }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <BCTopBar theme={theme}
        left={<div style={{ fontFamily: "'Newsreader', serif", fontSize: 28, color: theme.ink, paddingLeft: 6, letterSpacing: '-0.015em', whiteSpace: 'nowrap' }}>{T('activity.title')}</div>}
        right={<IconBtn name="check" theme={theme} onClick={onMarkRead} />}
      />

      <div style={{ flex: 1, overflowY: 'auto', padding: '10px 16px 140px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {items.map((n) => {
          ...
          return (
            <Card key={n.id} theme={theme} padded={false}
              onClick={() => n.group_id && onOpenGroup(n.group_id)}
              style={{ padding: '14px 16px', background: n.is_read ? theme.surface : theme.bg, border: `1px solid ${theme.softhair}` }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                <div style={{
                  width: 36, height: 36, borderRadius: 999, background: theme.chip, color: theme.ink,
                  ...
                  {!n.is_read && (
                    <div style={{
                      ...
                      background: accent, boxShadow: `0 0 0 2px ${theme.bg}`,
                    }} />
                  )}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{
                    ...
                    <div style={{
                      fontFamily: 'Be Vietnam Pro', fontSize: 10, color: theme.muted, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 500, flexShrink: 0,
                    }}>{typeLabel[n.type]}</div>
                    {group && <div style={{
                      fontFamily: 'Be Vietnam Pro', fontSize: 11, color: theme.muted,
                      whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                    }}>· {group.name}</div>}
                    <div style={{ flex: 1 }} />
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: theme.muted, letterSpacing: '0.04em', flexShrink: 0 }}>
                      {bcRelativeTimeL(n.created_at, lang).toUpperCase()}
                    </div>
                  </div>
                  <div style={{
                    fontFamily: 'Be Vietnam Pro', fontSize: 14.5, color: theme.ink, letterSpacing: '-0.005em',
                    lineHeight: 1.35,
                  }}>{message}</div>
                </div>
              </div>
...
function ProfileScreen({ state, theme, accent, T, lang, currency, onSetCurrency, onSetLang, onLogout }) {
  ...
  const rows = [
    { label: T('profile.language'),         value: lang === 'vi' ? 'Tiếng Việt' : 'English' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <BCTopBar theme={theme}
        left={<div style={{ fontFamily: "'Newsreader', serif", fontSize: 28, color: theme.ink, paddingLeft: 6, letterSpacing: '-0.015em', whiteSpace: 'nowrap' }}>{T('profile.title')}</div>}
        right={<IconBtn name="settings" theme={theme} />}
      />

      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 140px', display: 'flex', flexDirection: 'column', gap: 18 }}>
        <Card theme={theme}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <Avatar name={myName} seed={me.id} size={56} />
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{
                fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 17, color: theme.ink, letterSpacing: '-0.005em',
              }}>{myName}</div>
              <div style={{
                fontFamily: 'Be Vietnam Pro', fontSize: 13, color: theme.muted, marginTop: 2,
                whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
              }}>{me.email}</div>
            </div>
          </div>
        ...
        <div>
          <SectionLabel theme={theme} style={{ padding: '4px 4px 8px' }}>{T('profile.preferences')}</SectionLabel>
          <Card theme={theme} padded={false}>
            {rows.map((r, i) => (
              <div key={r.label} className="bc-tap"
                onClick={r.label === T('profile.language') ? () => onSetLang(lang === 'vi' ? 'en' : 'vi') : null}
                style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  padding: '14px 18px', borderTop: i === 0 ? 'none' : `1px solid ${theme.softhair}`,
                  cursor: r.label === T('profile.language') ? 'pointer' : 'default',
                }}>
                <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 15, color: theme.ink }}>{r.label}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 14, color: theme.muted }}>{r.value}</div>
        ...
        {archived.length > 0 && (
          <div>
            <SectionLabel theme={theme} style={{ padding: '4px 4px 8px' }}>{T('profile.archived_groups')}</SectionLabel>
            <Card theme={theme} padded={false}>
              {archived.map((g, i) => (
                <div key={g.id} style={{
                  display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px',
                  ...
                  <div style={{ flex: 1, fontFamily: 'Be Vietnam Pro', fontWeight: 500, fontSize: 14, color: theme.ink }}>{g.name}</div>
                  <div style={{ fontFamily: 'Be Vietnam Pro', fontSize: 11, color: theme.muted, letterSpacing: '0.06em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                    {T('profile.archived')}
                  </div>
                </div>
              ))}
        ...
        <div style={{ textAlign: 'center', padding: 12, fontFamily: 'Be Vietnam Pro', fontSize: 11, color: theme.muted, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          {T('common.app_footer')}
        </div>

        <button onClick={onLogout} className="bc-tap" style={{
          background: 'transparent', color: theme.neg, border: `1px solid ${theme.softhair}`,
...
function Stat({ label, value, theme }) {
  return (
    <Card theme={theme} padded={false} style={{ padding: '14px 14px' }}>
      <div style={{
        fontFamily: 'Be Vietnam Pro', fontSize: 10, color: theme.muted, letterSpacing: '0.1em', textTransform: 'uppercase',
      }}>{label}</div>
      <div style={{
        fontFamily: "'Newsreader', serif", fontSize: 30, color: theme.ink, lineHeight: 1.1,
        letterSpacing: '-0.015em', marginTop: 6, fontVariantNumeric: 'tabular-nums',
      }}>{value}</div>
    </Card>
  );
}
...
Object.assign(window, {
  GroupsHome, GroupDetail, AddExpense, SettleUp, ActivityScreen, ProfileScreen, GroupGlyph,
});

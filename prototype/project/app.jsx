...
function App() {
  ...
  const [draft, setDraft] = React.useState({
    description: '', amountStr: '', paid_by: null, category: 'food',
    date: '2026-05-22', splitMode: 'even', splitWith: null,
  });
  ...
  const startAddExpense = (gid) => {
    ...
    setDraft({
      description: '', amountStr: '', paid_by: myMid, category: 'food',
      date: '2026-05-22', splitMode: 'even', splitWith: null,
    });
  ...
  const saveExpense = (splitWith, perPerson) => {
    ...
    const exp = {
      id: eid, group_id: gid, paid_by: draft.paid_by,
      amount, currency: g.currency, description: draft.description || T('add.untitled'),
      category: draft.category, date: draft.date,
      created_at: new Date().toISOString(), created_by: 'u_me',
    };
    const splits = splitWith.map((mid, i) => ({
      id: `s_${eid}_${i}`, expense_id: eid, member_id: mid,
    ...
    const notif = {
      id: 'n_' + Date.now(), user_id: 'u_me', group_id: gid, type: 'expense_added',
      params: { actor: actorName, what: exp.description, amount: `${sym}${amount.toFixed(2)}` },
      is_read: false, created_at: exp.created_at,
    };
    setState((s) => ({
      ...s,
  ...
  const saveSettlement = ({ from, to, amount }) => {
    ...
    const st = {
      id: sid, group_id: gid, from_member: from, to_member: to,
      amount, currency: g.currency,
      settled_at: new Date().toISOString(), created_by: 'u_me',
    };
    const fromM = bcMember(state, from);
    const toM = bcMember(state, to);
    const sym = bcCurrencySymbol(g.currency);
    const notif = {
      id: 'n_' + Date.now(), user_id: 'u_me', group_id: gid, type: 'settlement_sent',
      params: {
        from: fromM.user_id === 'u_me' ? 'You' : fromM.display_name,
        to:   toM.user_id === 'u_me' ? 'You' : toM.display_name,
        amount: `${sym}${amount.toFixed(2)}`,
      },
      is_read: false, created_at: st.settled_at,
    };
    setState((s) => ({
      ...s,
  ...
  if (route.name === 'tab') {
    if (tab === 'home') {
      ...
      screen = <GroupsHome state={state} theme={theme} accent={accent} T={T} lang={lang}
        onOpenGroup={openGroup} onNewGroup={() => {}} />;
    } else if (tab === 'activity') {
      screenLabel = '06 Activity';
      screen = <ActivityScreen state={state} theme={theme} accent={accent} T={T} lang={lang}
        onMarkRead={markAllRead}
        onOpenGroup={(gid) => { setTab('home'); openGroup(gid); }} />;
    } else if (tab === 'profile') {
      screenLabel = '07 Profile';
      screen = <ProfileScreen state={state} theme={theme} accent={accent} T={T} lang={lang}
        currency={tw.currency} onSetCurrency={(v) => setTweak('currency', v)}
        onSetLang={(v) => setTweak('lang', v)}
        onLogout={onLogout} />;
    }
  } else if (route.name === 'group') {
    screenLabel = '02 Group';
    screen = <GroupDetail state={state} theme={theme} accent={accent} T={T} lang={lang} groupId={route.groupId}
      onBack={backToTab}
      onAddExpense={() => startAddExpense(route.groupId)}
      onSettleUp={() => startSettle(route.groupId)} />;
  } else if (route.name === 'add') {
    screenLabel = '03 Add expense';
    screen = <AddExpense state={state} theme={theme} accent={accent} T={T} lang={lang} groupId={route.groupId}
      draft={draft} setDraft={setDraft}
      onBack={() => setRoute({ name: 'group', groupId: route.groupId })}
      onSave={saveExpense} />;
  } else if (route.name === 'settle') {
    screenLabel = '04 Settle up';
    screen = <SettleUp state={state} theme={theme} accent={accent} T={T} groupId={route.groupId}
      onBack={() => setRoute({ name: 'group', groupId: route.groupId })}
  ...
  const inner = (
    ...
    >
      {loggedIn ? screen : (
        <LandingScreen theme={theme} accent={accent} T={T} lang={lang}
          onLogin={onLogin} onDemo={onDemo} />
      )}
      {showBottomBar && (
        <BCBottomBarLocalized active={tab} onChange={setTab} theme={theme} unread={unread} T={T} />
      )}
      {showAuth && (
        <AuthSheet theme={theme} accent={accent} T={T}
  ...
  return (
    <React.Fragment>
      {tw.showFrame ? (
        ...
        <div style={{
          width: 402, height: 874, borderRadius: 32, overflow: 'hidden', position: 'relative',
          background: theme.bg, boxShadow: '0 30px 60px rgba(0,0,0,0.18), 0 0 0 1px rgba(0,0,0,0.06)',
        }}>{inner}</div>
      )}

      <TweaksPanel title="Tweaks" noDeckControls>
        <TweakSection label="Language">
          <TweakRadio label="App language" value={lang}
            options={[
        ...
        <TweakSection label="Appearance">
          ...
          <TweakToggle label="Device frame" value={tw.showFrame}
            onChange={(v) => setTweak('showFrame', v)} />
        </TweakSection>

        <TweakSection label="Auth">
          <TweakToggle label="Signed in" value={loggedIn}
            onChange={(v) => setTweak('loggedIn', v)} />
        </TweakSection>

        <TweakSection label="Jump to">
          <TweakSelect label="Screen" value={JSON.stringify({ route: route.name, tab })}
            options={[
              { label: 'Groups (home)',     value: JSON.stringify({ route: 'tab', tab: 'home' }) },
              { label: 'Group — Rome trip', value: JSON.stringify({ route: 'group', tab: 'home', gid: 'g_rome' }) },
              { label: 'Group — Apt 4B',    value: JSON.stringify({ route: 'group', tab: 'home', gid: 'g_apt' }) },
              { label: 'Add expense',       value: JSON.stringify({ route: 'add', tab: 'home', gid: 'g_rome' }) },
              { label: 'Settle up',         value: JSON.stringify({ route: 'settle', tab: 'home', gid: 'g_rome' }) },
              { label: 'Activity',          value: JSON.stringify({ route: 'tab', tab: 'activity' }) },
              { label: 'Profile',           value: JSON.stringify({ route: 'tab', tab: 'profile' }) },
            ]}
            onChange={(v) => {
              ...
              else if (parsed.route === 'add') { startAddExpense(parsed.gid); }
              else if (parsed.route === 'settle') { startSettle(parsed.gid); }
            }} />
        </TweakSection>
      </TweaksPanel>
    </React.Fragment>
...
function BCBottomBarLocalized({ active, onChange, theme, unread, T }) {
  ...
  const tabs = [
    ...
    { k: 'activity', label: T('nav.activity'), icon: 'activity', badge: unread },

  ...
  return (
    ...
    >
      {keys.map((k) => (
        ...
        >
          {k === 'del' ? <BCIcon name="back" size={20} color={theme.ink} stroke={1.6} /> : k}
        </button>
      ))}
    </div>
  )
  ...
  return (
    ...
    >
      ...
      >
        ...
        {display.includes('.') && <span style={{ fontSize: size * 0.58, color: theme.muted }}>.{display.split('.')[1]}</span>}
      </span>
    </div>
  )
}
  ...
  return (
    <button
      ...
      style={{
        ...
        border: 'none',
        cursor: 'pointer',
        padding: label ? '14px 22px 14px 18px' : 14,
        borderRadius: 999,
        display: 'inline-flex',
        alignItems: 'center',
        gap: 8,

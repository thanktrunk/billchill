...
export const expensesRelations = relations(expenses, ({ one, many }) => ({
  ...
  paidByMember: one(groupMembers, { fields: [expenses.paidBy], references: [groupMembers.id] }),
...
export const settlementsRelations = relations(settlements, ({ one }) => ({
  ...
  fromMemberRef: one(groupMembers, { fields: [settlements.fromMember], references: [groupMembers.id], relationName: "settlementFrom" }),
  toMemberRef: one(groupMembers, { fields: [settlements.toMember], references: [groupMembers.id], relationName: "settlementTo" }),
  createdByUser: one(users, { fields: [settlements.createdBy], references: [users.id] }),
}));

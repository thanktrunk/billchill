...
export function fmtCurrencyShort(value: number, symbol: string) {
  ...
  if (value >= 1_000) return `${symbol}${(value / 1_000).toFixed(1).replace(/\.0$/, '')}K`
...
export function BCDonutChart({ data, symbol, totalLabel = 'Total' }: { data: DonutItem[]; symbol: string; totalLabel?: string }) {
    ...
    <>
      <div className="relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          <span className="font-serif text-[22px] leading-tight tracking-[-0.02em] text-(--bc-ink)">{fmtCurrency(total, symbol)}</span>
        </div>
      </div>
    ...
    <ResponsiveContainer width="100%" height={Math.max(data.length * 44, 80)}>
      <BarChart data={data} layout="vertical" margin={{ left: 0, right: rightMargin, top: 4, bottom: 4 }}>
        ...
        <YAxis
          ...
          dataKey="name"
          width={labelWidth}
          tick={{ ...YAXIS_TICK, fontSize: nameFontSize }}
          axisLine={false}
          tickLine={false}
        />
        <Tooltip
        ...
        >
          {hasPerCellColor && data.map((entry, i) => <Cell key={i} fill={entry.color ?? fill} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  )
    ...
    for (const anchor of placed) {
      for (let a = 0; a < 72; a++) {
        ...
        for (const p of placed) {
          if (Math.hypot(x - p.x, y - p.y) < r + p.r - 0.5) {
            valid = false
            break
          }
        }
...
export function BCBubbleChart({ data, symbol, color = '--bc-pos' }: { data: BubbleItem[]; symbol: string; color?: string }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[24px]">
      <div className="pointer-events-none absolute -left-14 top-8 h-36 w-36 rounded-full bg-emerald-300/12 blur-2xl" />
      <div className="pointer-events-none absolute -right-10 bottom-4 h-32 w-32 rounded-full bg-emerald-200/8 blur-2xl" />
      <svg viewBox={viewBox} className="relative z-10 w-full h-auto">
        {placed.map((p, i) => {
          const isFocus = i === 0
          const nameFz = Math.max(9, p.r * 0.24)
          const amtFz = Math.max(10, p.r * 0.34)
          const bubbleOpacity = isFocus ? Math.min(88, p.opacity + 14) : Math.max(34, p.opacity - 8)
          const baseFill = `color-mix(in srgb, var(${color}) ${bubbleOpacity}%, transparent)`
          const gap = Math.max(2, p.r * 0.03)
          return (
            <g key={p.name}>
              <circle cx={p.x} cy={p.y} r={p.r} fill={baseFill} stroke="rgb(255 255 255 / 0.08)" strokeWidth={isFocus ? 1.5 : 1} />
              <text
                x={p.x}
                y={p.y - amtFz / 2 - gap}
                textAnchor="middle"

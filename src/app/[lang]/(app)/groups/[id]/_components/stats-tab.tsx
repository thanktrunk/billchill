...
export function StatsTab({
  ...
  const spentByData = useMemo(() => {
    ...
    for (const s of splits) {
      totals[s.memberId] =
        (totals[s.memberId] ?? 0) + parseFloat(s.shareAmount);
    }
    return Object.entries(totals)
      .map(([id, value]) => ({
        name: memberById.get(id)?.displayName ?? id,
  ...
  const settlementProgress = useMemo(() => {
    ...
    const outstanding = balances.reduce(
      (s, b) => s + (b.balance > 0.005 ? b.balance : 0),
      0,
    );
  ...
  if (expenses.length === 0) {
    return (
      <div className="flex items-center justify-center py-20">
        <p className="font-sans text-[14px] text-(--bc-muted)">
          {tGroup("stats_no_data")}
        </p>
      </div>
    );
  }
        ...
        <BCCard padded={false} className="px-4 py-4">
          <div className="flex justify-between items-baseline mb-3">
            <div className="flex flex-col gap-0.5">
              <span className="font-sans text-[11px] text-(--bc-muted) uppercase tracking-[0.08em]">
                {tGroup("stats_settled")}
              </span>
            ...
            <div className="font-serif text-[28px] tracking-[-0.02em] text-(--bc-muted)">
              {settlementProgress.pct}%
            </div>
            <div className="flex flex-col gap-0.5 items-end">
              <span className="font-sans text-[11px] text-(--bc-muted) uppercase tracking-[0.08em]">
                {tGroup("stats_outstanding")}
              </span>
